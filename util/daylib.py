...
class daylib():
    ...
    @classmethod
    def currentTime(cls, local_tz_str=None):
        dt_utc = dt.utcnow()
        dt_offset = cls.localize_dt(dt_utc, local_tz_str)
        return dt_offset
    ...
    @classmethod
    def dt_to_strYMDHMformat(cls, dt_obj):
        return dt_obj.strftime('%Y-%m-%d %H:%M')
    ...
    @classmethod
    def strYMDHM_to_dt(cls, str_dt):
        return dt.strptime(str_dt, '%Y%m%d%H%M')
    ...
    def str_utc_to_dt_offset(cls, str_utc, local_tz_str=None,is_T=True,is_ms=True, is_Z=True):
        ...
        dt_local_tz = cls.localize_dt(dt_utc, local_tz_str)

        return dt_local_tz

    @classmethod
    def localize_dt(cls, dt_utc, local_tz_str=None):
